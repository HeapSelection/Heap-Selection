...
static __inline void sort5(Node **nodes) {
	SWAP(0, 1);
	SWAP(1, 2);
	SWAP(0, 1);
	SWAP(2, 3);
...
void OptimizedCLRSMedianPartition::partition(int k, vector<Node *> &nodes) {
	size_t low = 0;
...
void OptimizedCLRSMedianPartition::SELECT(vector<Node *> &nodes, size_t low, size_t high) {
