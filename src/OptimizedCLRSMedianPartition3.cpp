...
static __inline void sort53(Node **nodes) {
	SWAP(0, 1);
	SWAP(2, 3);
	SWAP(0, 2);
	SWAP(1, 3);
...
void OptimizedCLRSMedianPartition3::partition(int k, vector<Node *> &nodes) {
	size_t low = 0;
...
void OptimizedCLRSMedianPartition3::SELECT(vector<Node *> &nodes, size_t low, size_t high) {
